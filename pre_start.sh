#!/bin/bash
mkdir -p $HF_HOME
mkdir -p $COMFY_MODELS_INSTALL_DIR/{checkpoints,clip,clip_vision,configs,controlnets,embeddings,loras,upscale_models,vae,diffusion_models,unet,text_encoders}
mkdir -p $WORKSPACE_FOLDER/{comfy_user,comfy_outputs,comfy_inputs}

if [[ "$BAKED" != "yes" ]]; then
    ./install_comfyui.sh $COMFYUI_INSTALL_DIR $COMFYUI_VERSION
fi

 # --output-directory, --input-directory, --user-directory
nohup python $COMFYUI_INSTALL_DIR/main.py --listen --port 3000 --user-directory $WORKSPACE_FOLDER/comfy_user --output-directory $WORKSPACE_FOLDER/comfy_outputs --input-directory $WORKSPACE_FOLDER/comfy_inputs & >> /dev/stdout 2>&1 &

# check if environment variable is set, if so, use it's value to run load_models.sh
if [ -n "$MODEL_SET_ONSTARTUP" ]; then
    cd /
    load_models.sh $MODEL_SET_ONSTARTUP
fi

# Ensure the process started
sleep 2
if ! pgrep -f "python.*main.py.*--port.*3000" > /dev/null; then
    echo "Failed to start ComfyUI"
    exit 1
fi